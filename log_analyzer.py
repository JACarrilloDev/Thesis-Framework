...
def rolling_metrics(df: pd.DataFrame, window: int) -> pd.DataFrame:
    ...
    cols = base_cols + ([] if IGNORE_SUCCESS_HITS else ["nav_hits_sum"])
    for col in cols:
        if col in r.columns:
...
def summarize_slice(df: pd.DataFrame, lo: int, hi: int) -> Dict[str, float]:
    ...
    # Totals/lasts
    if "timesteps_total" in s.columns and s["timesteps_total"].notna().any():
        put("timesteps_total_last", s["timesteps_total"].dropna().iloc[-1])
    if "wall_time_s_total" in s.columns and s["wall_time_s_total"].notna().any():
        wt = s["wall_time_s_total"].dropna().iloc[-1]
        put("wall_time_s_total_last", wt)
        put("wall_time_h_total_last", wt / 3600.0)
    if "iter_time_s" in s.columns and s["iter_time_s"].notna().any():
        its = s["iter_time_s"].fillna(0).sum()
        put("iter_time_s_sum", its)
        put("iter_time_h_sum", its / 3600.0)
    return out
...
def print_summary(df: pd.DataFrame, tail_n: int = 200):
    ...
    if (not IGNORE_SUCCESS_HITS) and ("nav_hits_sum" in df.columns):
        mask_any = df["nav_hits_sum"].notna()
        if mask_any.any():
            pos = int(np.flatnonzero(mask_any.values)[0])
            ti = int(df["training_iteration"].iloc[pos])
...
def plot_series(df: pd.DataFrame, outdir: str, shifts: List[Tuple[int, str]], window: int):
    ...
    def annotate(ax):
        for ti, msg in shifts:
            is_curr = ("Curriculum boundary" in msg)
            if is_curr:
                # High-contrast curriculum boundary
                ax.axvline(ti, color="tab:red", linestyle="-", linewidth=2.2, alpha=0.95, zorder=10)
                # subtle band to aid visibility
                ax.axvspan(ti - 0.2, ti + 0.2, color="tab:red", alpha=0.05, zorder=0)
                ax.text(
                    ti, ax.get_ylim()[1], msg,
                    rotation=90, va="top", ha="right",
                    fontsize=9, color="tab:red", alpha=0.9,
                    bbox=dict(facecolor="white", edgecolor="tab:red", linewidth=0.8, boxstyle="round,pad=0.15", alpha=0.75),
                )
            else:
                # Other shift annotations (subtle)
                ax.axvline(ti, color="gray", linestyle="--", linewidth=1.0, alpha=0.4, zorder=5)
                ax.text(ti, ax.get_ylim()[1], msg, rotation=90, va="top", ha="right", fontsize=8, alpha=0.6)
        # Add legend key for curriculum boundary once per axis
        _, labels = ax.get_legend_handles_labels()
        if "Curriculum boundary" not in labels:
            ax.plot([], [], color="tab:red", linewidth=2.2, label="Curriculum boundary")
    ...
    # Hits (nav collisions)
    if (not IGNORE_SUCCESS_HITS) and ("nav_hits_sum" in df.columns) and has_data("nav_hits_sum"):
        plt.figure(figsize=(11, 5))
    ...
    # Success rates (skip if ignoring)
    if not IGNORE_SUCCESS_HITS:
...
def write_markdown_report(df: pd.DataFrame, outdir: str, shifts: List[Tuple[int, str]], window: int, tail_n: int = 200):
    ...
    lines.append("")
    lines.append("## Plots")
    plot_names = ["reward_and_len.png", "diagnostics.png", "throughput.png", "episodes_scatter.png"]
    if not IGNORE_SUCCESS_HITS:
        plot_names += ["nav_hits.png", "success_rate.png"]
...
def analyze_one(df_run: pd.DataFrame, outdir: str, window: int, tail_n: int, curriculum_boundaries: Optional[List[int]], no_plots: bool):
    ...
    os.makedirs(outdir, exist_ok=True)
    # If ignoring success/hits, proactively remove stale plots from previous runs
    if IGNORE_SUCCESS_HITS:
        for name in [
            "nav_hits.png",
            "success_rate.png",
            "full_success_mean.png",
            "success_mean.png",
            "success_phase1_mean.png",
            "success_qtr_mean.png",
        ]:
            p = os.path.join(outdir, name)
            try:
                if os.path.exists(p):
...
def main():
    ...
    for rid in run_ids:
        ...
        if args.clean_outdir and os.path.isdir(outdir):
